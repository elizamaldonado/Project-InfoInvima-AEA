...
class ArticuloProvider {
  Future<List<ArticuloModel>> obtenerArticulos() async {
    ...
    try {
      var response = await client
          .get(Uri.https('datos.gov.co', '/resource/i7cb-raxc.json'));
      var decodedResponse =
  ...
  Future<List<ArticuloModel>> filtrarArticulos(
      String nombre, String forma) async {
    List<ArticuloModel> articulos = [];
    var client = http.Client();
    try {
      ...
      var endpoint =
          "api/id/i7cb-raxc.json?$query=select *, :id where (contains(upper(`producto`), upper('$nombre')) and (upper(`formafarmaceutica`) = upper('$forma')) )";
